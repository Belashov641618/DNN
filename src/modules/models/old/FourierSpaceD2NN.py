...
from modules.layers.old.LensLayer import LensLayer
...
class FourierSpaceD2NN(torch.nn.Module):

    _DefaultPath        : str = 'src/modules/models/autosave/'
    _DefaultFilename    : str = 'FourierSpaceD2NN.data'
    @staticmethod
    def load(file_name:str=None):
        if file_name is None:
            file_name = FourierSpaceD2NN._DefaultPath + FourierSpaceD2NN._DefaultFilename
        try:
    ...
    _LensModule                     : torch.nn.Module
    _LensPropagationModule          : torch.nn.Module
    ...
    def FinalizeChanges(self):
        modules_to_finalize_list = [self._LensModule, self._LensPropagationModule, self._DetectorsModule, self._HeightsMaskPropagationModule] + list(self._HeightsMaskModuleList)
    ...
    def WaveLength(self, wave_length:Union[float,Iterable,torch.Tensor]):
        ...
        synchronize_modules = [self._LensModule, self._LensPropagationModule, self._DetectorsModule, self._HeightsMaskPropagationModule] + list(self._HeightsMaskModuleList)
    ...
    def SpaceReflection(self, space_reflection:Union[float,Iterable,torch.Tensor]):
        ...
        synchronize_modules = [self._LensModule, self._LensPropagationModule, self._HeightsMaskPropagationModule] + list(self._HeightsMaskModuleList)
        for module in synchronize_modules:
    ...
    _LensBorderLength : float
    @property
    def LensBorderLength(self):
        return self._LensBorderLength
    @LensBorderLength.setter
    def LensBorderLength(self, lens_propagation_border_length:float):
        self._LensBorderLength = lens_propagation_border_length
        synchronize_modules = [self._LensPropagationModule]
        for module in synchronize_modules:
            if hasattr(module, 'BorderLength'):
                setattr(module, 'BorderLength', self._LensBorderLength)
    ...
    def FocusLength(self, focus_length:float):
        ...
        synchronize_modules = [self._LensModule]
        for module in synchronize_modules:
            if hasattr(module, 'FocusLength'):
                setattr(module, 'FocusLength', self._FocusLength)
        synchronize_modules = [self._LensPropagationModule]
        for module in synchronize_modules:
            if hasattr(module, 'DiffractionLength'):
                setattr(module, 'DiffractionLength', self._FocusLength)
    ...
    def DiffractionLengthAsParameter(self, mode:bool=True):
        if hasattr(self._HeightsMaskPropagationModule, 'DiffractionLengthAsParameter'):
            ...
            raise AttributeError("\033[31m\033[1m{}".format('FourierSpaceD2NN: HeightsMaskPropagationModule must have attribute "DiffractionLengthAsParameter"!'))
    ...
    def __init__(self, masks_count:int=5, wave_length:Union[float,Iterable,torch.Tensor]=600*nm, space_reflection:Union[float,Iterable,torch.Tensor]=1.0, mask_reflection: Union[float, Iterable, torch.Tensor] = 1.5, plane_length:float=1.0*mm, pixels_count:int=20, up_scaling:int=4, mask_interspacing:float=20.0*mm, mask_propagation_border_length:float=0.0, lens_propagation_border_length:float=0.0, smoothing_matrix:Union[None,Iterable,torch.Tensor]=None, focus_length:float=10*mm, detectors_masks:Union[str,Iterable,torch.Tensor]='Polar'):
        ...
        """
        super(FourierSpaceD2NN, self).__init__()
        ...
        self.LensBorderLength   = lens_propagation_border_length
        self.FocusLength        = focus_length
    ...
    def forward(self, field:torch.Tensor, record_history:bool=False):
        ...
        if record_history:
            ...
            length += self._FocusLength
            history.append(('Амплитуда перед входной линзой' + ': ' + Format.Engineering(length, 'm', 1), (torch.abs(field) ** 1).cpu()))
            field = self._LensModule(field)
            field = self._LensPropagationModule(field)
            length += self._FocusLength
            history.append(('Амплитуда перед маской №' + str(1) + ': ' + Format.Engineering(length, 'm', 1), (torch.abs(field) ** 1).cpu()))
            ...
            field = self._LensPropagationModule(field)
            length += self._FocusLength
            history.append(('Амплитуда перед выходной линзой' + ': ' + Format.Engineering(length, 'm', 1), (torch.abs(field) ** 1).cpu()))
            field = self._LensModule(field)
            field = self._LensPropagationModule(field)
            length += self._FocusLength
