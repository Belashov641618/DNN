...
class AbstractMaskLayer(AbstractLayer):
    ...
    _parameters_ : torch.nn.Parameter
    def _mask(self):
        return self._scale()
    def _recalc_parameters(self):
        with torch.no_grad():
            if hasattr(self, '_parameters_'):
                device = self._parameters_.device
                ParametersCopy = self._parameters_.clone()
                if ParametersCopy.size(0) < self._pixels:
                    ...
                    nx1, nx2 = int(ParametersCopy.size()[0]/2 - self._pixels/2), int(ParametersCopy.size()[0]/2 + self._pixels/2) + 1
                    ny1, ny2 = int(ParametersCopy.size()[1]/2 - self._pixels/2), int(ParametersCopy.size()[1]/2 + self._pixels/2) + 1
                    if nx2 - nx1 > self._pixels: nx2 -= 1
                    if ny2 - ny1 > self._pixels: ny2 -= 1
                    Parameters = ParametersCopy[nx1:nx2, ny1:ny2]
                self._parameters_ = torch.nn.Parameter(Parameters.to(device))
            else:
                self._parameters_ = torch.nn.Parameter(torch.normal(mean=0, std=10, size=(self._pixels, self._pixels), dtype=self._accuracy.tensor_float))
    ...
    def _normalize(self):
        if self._normalization_type == sigmoid_normalization:
            return torch.sigmoid(self._parameters_)
        elif self._normalization_type == sinus_normalization:
            return torch.sin(0.5 * self._normalization_parameters[0] * self._parameters_ / torch.pi)
